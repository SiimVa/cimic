...
if st.button("Käivita analüüs"):
    if not all(pts_mgrs):
        st.error("Palun sisesta kõik 4 MGRS punkti.")
    else:
        try:
            # ============================================================
            ...
            for p_mgrs, (x, y) in zip(pts_mgrs, pts_xy):
            ...
            st.write(f"BBox-alusel loetud katastriüksusi: {len(ky)}")

            ky_clip = gpd.clip(ky, analysis_gdf).copy()
            st.write(f"Täpse analüüsialaga lõigatud katastriüksusi: {len(ky_clip)}")
            ...
            if missing:
                st.write("Kõik olemasolevad veerud:")
                st.write(list(ky_clip.columns))
                raise ValueError(f"Puuduvad vajalikud veerud: {missing}")
            ...
            def standardize_omvorm(value):
                ...
                elif "omandi ulatus" in v and "selgit" in v:
                    return "Omandi ulatus selgitamisel"
                elif "avalik" in v and "õiguslik" in v:
            ...
            for om in present_omandivormid:
                st.write(f"- {om}")

            if "Määramata" in ky_clip["omvorm_std"].unique():
            ...
            csv_data = result_df.to_csv(index=False, encoding="utf-8-sig", sep=";")
            st.download_button(
            ...
            st.dataframe(area_summary_df)

            st.write(f"Analüüsiala kogupindala: {analysis_area_km2:.6f} km²")
            ...
            # Save map to temp file and display
            ...
            os.unlink(map_filename)  # Clean up temp file
